...
When a developer says **"install cogen memory"** — follow these steps exactly:

**Step 1:** Create all Cogen files listed in the Files to Create section below.
...
**Step 3:** Ask the user this exactly:

> **Cogen is installed! Choose your memory mode:**
>
> 1. **Local memory** — works on your machine, fast, private, no setup needed
> 2. **Cloud memory (Turso)** — works across ALL AI sessions including sandboxed ones like Manus and other AI agent that accept custom Skills like Claude Code, Cursor, Gemini CLI etc. Your memory persists everywhere.
>
> Which would you like?

**If user picks 1 (Local):**
→ Done. SQLite is already configured. Tell the user to set `OPENROUTER_API_KEY` or use there own AI model if they want to and they're ready. Also tell the user that this is mostly used in CLI or terminal environment 

**If user picks 2 (Cloud / Turso):**
→ Tell the user:
> Go to **turso.tech** → Sign up free → Create a database called `cogen-memory` → Go to the database settings → Copy the **Database URL** and **Auth Token** → Paste both here.

→ Wait for the user to paste the URL and token.

→ Set environment variables:
```bash
export TURSO_DATABASE_URL=their_url
export TURSO_AUTH_TOKEN=their_token
```

→ Create `memory_cloud.py` using the code in the Files to Create section below.

→ The auto-detection in `memory.py` will switch to cloud mode automatically once `TURSO_DATABASE_URL` is set.

→ Run the verification test again to confirm the cloud connection works.

→ Tell the user: **"Cloud memory active. Your identity now persists across every AI session."**
...
Only `requests` and `sqlite3` (stdlib) are required for local mode. For cloud mode, `libsql-experimental` is auto-installed by `memory_cloud.py` on first run.

---

## Memory Modes

| Mode | Storage | Works in | Setup |
|------|---------|----------|-------|
| Local | SQLite file (`memory.db`) | Local machine, Termux, laptop | Zero config |
| Cloud | Turso (SQLite cloud) | Everywhere — Manus, Claude Code, any sandbox | Free Turso account |

Auto-detected via environment variables. If `TURSO_DATABASE_URL` is set → cloud mode. Otherwise → local SQLite mode.
...
_(Only needed if user chose Cloud mode. Auto-installs `libsql-experimental` on first run.)_
...
try:
    ...
    import subprocess
    subprocess.run(['pip', 'install', 'libsql-experimental'], check=True)
    import libsql_experimental as libsql
...
def _get_conn():
    ...
    conn  = libsql.connect('cogen-memory', sync_url=url, auth_token=token)
    conn.sync()
    return conn
    ...
    return [
        {
            ...
            'importance': r[3], 'emotion': r[4],
            'topics': json.loads(r[5] or '[]'),
            'timestamp': r[6], 'conflict': r[7],
            'belief_score': r[8]
        }
        for r in rows
    ]
...
# Auto-detect storage mode — cloud if TURSO_DATABASE_URL is set, else local SQLite
TURSO_URL = os.environ.get('TURSO_DATABASE_URL')

if TURSO_URL:
    # Cloud mode — delegates everything to memory_cloud.py
    from memory_cloud import save_message, load_memory, clear_memory
else:
    # Local mode — SQLite (default, zero config)
    import sqlite3
    import json
    from datetime import datetime
    from importance import calculate_importance
...
├── memory.py         ✓ Auto-detects local vs cloud mode on import
├── memory_cloud.py   ✓ Cloud memory via Turso — persists across sandboxed AI sessions
...
Local mode: no external packages beyond `requests`. SQLite database created automatically on first run.
Cloud mode: `libsql-experimental` auto-installed by `memory_cloud.py` on first import. Set `TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN` to activate.
